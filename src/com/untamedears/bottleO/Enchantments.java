...
public class Enchantments {
    ...
    public Enchantments(int level, Material material) {
        ...
        // triangular-distributed random int between 0 and enchantability with most probable value enchantability / 2
            int modifiedEnchantability = (rnd.nextInt(enchantability / 2 + 1) + rnd.nextInt(enchantability / 2 + 1));
                 
        // level added to one plus the previously generated int 
            int augmentedLevel = modifiedEnchantability + 1 + level; 
             
        // uniformly-distributed random double between 0.75 and 1.25
            double multiplier = Math.random() / 2.0 + 0.75; 
             
        // level after being modified by the enchantability and multiplier modifiers
            int modified_level = Math.round((float)(augmentedLevel * multiplier)); 
        ...
        case BOW:
            ...
            for (int i = 0; i < 10; i++) {
                if (modified_level >= 49 && modified_level <= 64) {
                    this.add(Enchantment.ARROW_DAMAGE, 5);
                }
                else if (modified_level >= 37 && modified_level <= 52) {
                    this.add(Enchantment.ARROW_DAMAGE, 4);
                }
                else if (modified_level >= 25 && modified_level <= 40) {
                    this.add(Enchantment.ARROW_DAMAGE, 3);
                }
                else if (modified_level >= 13 && modified_level <= 28) {
        ...
        do {
            ...
            modified_level = modified_level / 2;
